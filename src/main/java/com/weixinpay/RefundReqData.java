...
public class RefundReqData {
     ...
     */
    public RefundReqData(String transactionID,String outTradeNo,String outRefundNo,int totalFee,int refundFee) throws IllegalAccessException{
        ...
        setOut_refund_no(randomStringByLength);

        setTotal_fee(totalFee);

        setRefund_fee(refundFee);

        setOp_user_id(Configure.getMch_id());

        //随机字符串，不长于32 位
        setNonce_str(randomStringByLength);
        ...
        parameters.put("out_refund_no", nonce_str);
        parameters.put("out_trade_no", out_trade_no);
        parameters.put("refund_fee", refund_fee);
        parameters.put("transaction_id", transaction_id);
        parameters.put("total_fee", total_fee);

        String sign = Signature.getMapSign(parameters);
